...
void ball_detect(){
  ...
  Mat morph_mask = getStructuringElement(MORPH_RECT,Size(5,5),Point(1,1));
  GaussianBlur(blue_mask, blue_mask, cv::Size(5,5),0);
  //dilate(blue_mask,blue_mask,morph_mask,Point(-1,-1),4);
  //erode(blue_mask,blue_mask,morph_mask,Point(-1,-1),4);  // smoothing

  GaussianBlur(green_mask, green_mask, cv::Size(5,5),0);
  //dilate(green_mask,green_mask,0);
  //erode(green_mask,green_mask,0);  // smoothing

  GaussianBlur(red_mask, red_mask, cv::Size(5,5),0);
  //dilate(red_mask,red_mask,0);
  //erode(red_mask,red_mask,0);  // smoothing
...
void depthCallback(const sensor_msgs::ImageConstPtr& msg){
  try{
    ...
    depth_img_cv = cv_bridge::toCvShare(msg, sensor_msgs::image_encodings::TYPE_32FC1);//sensor_msgs::image_encodings::TYPE_16UC1
    depth_img_cv->image.convertTo(buffer_depth, CV_8U, 40);
    Mat disp_depth;
    ...
    Scalar mean_value;
    mean_value = mean(buffer_depth, green_mask);

    float mean_val = sum(mean_value)[0];

    ROS_INFO("%f", mean_val);
    imshow("depth", buffer_depth);
